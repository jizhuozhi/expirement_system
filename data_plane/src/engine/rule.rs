...
/// Field data types supported in rule evaluation
...
impl Node {
    /// Validate rule node against field type definitions
    pub fn validate(&self, field_types: &HashMap<String, FieldType>) -> Result<()> {
        match self {
            ...
            Node::Field { field, op, values } => {
                // Check field exists in type map
                let field_type = field_types.get(field).ok_or_else(|| {
                    ExperimentError::InvalidRule(format!(
                        "Field '{}' not found in field type map",
                        field
                    ))
                })?;
                ...
                if values.is_empty() {
                    return Err(ExperimentError::InvalidRule(format!(
                        "Field '{}' operator {:?} requires at least one value",
                        field, op
                    )));
                }
                ...
                match op {
                    Op::And | Op::Or | Op::Not => {
                        return Err(ExperimentError::InvalidRule(format!(
                            "Boolean operator {:?} cannot be used in Field node",
                            op
                        )));
                    }
                    _ => {}
    ...
    /// Evaluate rule node against context data
    pub fn evaluate(
        &self,
        ...
        match self {
            ...
            Node::Field { field, op, values } => {
                ...
                let field_value = ctx.get(field).ok_or_else(|| {
                    ExperimentError::InvalidRule(format!("Field '{}' not found in context", field))
                })?;
    ...
    match (field_type, value) {
        ...
        (FieldType::SemVer, Value::String(s)) => {
            ...
            if s.split('.').count() >= 2 {
                ...
                Err(ExperimentError::InvalidRule(format!(
                    "Field '{}' value '{}' is not a valid semver",
                    field_name, s
                )))
            }
        }
        _ => Err(ExperimentError::InvalidRule(format!(
...
/// Evaluate field operation against values
fn evaluate_field_op(
    ...
    match op {
        ...
        Op::And | Op::Or | Op::Not => Err(ExperimentError::InvalidRule(format!(
            "Boolean operator {:?} cannot be used in field comparison",
            op
        ))),
    }
}
...
fn compare_values(
    ...
    match field_type {
        ...
        FieldType::Bool => match (left.as_bool(), right.as_bool()) {
            (Some(l), Some(r)) => Ok(l.cmp(&r)),
            _ => Err(ExperimentError::InvalidRule(
                "Bool comparison requires boolean values".to_string(),
            )),
        },
        FieldType::SemVer => match (left.as_str(), right.as_str()) {
...
/// Simple wildcard pattern matching with * support
...
mod tests {
    ...
    fn test_evaluate_eq() {
        let field_types = setup_field_types();
        let ctx = [("country".to_string(), json!("US"))].into_iter().collect();
    ...
    fn test_evaluate_neq() {
        let field_types = setup_field_types();
        let ctx = [("country".to_string(), json!("CN"))].into_iter().collect();
    ...
    fn test_evaluate_gte() {
        let field_types = setup_field_types();
        let ctx = [("age".to_string(), json!(25))].into_iter().collect();
    ...
    fn test_evaluate_like() {
        let field_types = setup_field_types();
        let ctx = [("user_id".to_string(), json!("user_12345"))]
            .into_iter()
            .collect();
    ...
    fn test_evaluate_nested() {
        ...
        // Test nested rule: (country=US AND age>=18) OR premium=true
        let node = Node::Or {
            children: vec![
                Node::And {
                    children: vec![
    ...
    fn test_compare_semver() {
        assert_eq!(
            compare_semver("1.2.3", "1.2.3").unwrap(),
            std::cmp::Ordering::Equal
        );
        assert_eq!(
            compare_semver("1.2.4", "1.2.3").unwrap(),
            std::cmp::Ordering::Greater
        );
        assert_eq!(
            compare_semver("1.2.2", "1.2.3").unwrap(),
