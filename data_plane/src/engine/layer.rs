...
fn convert_buckets_to_ranges(
    ...
    let mut boundaries: Vec<(u32, String)> = buckets.iter().map(|(k, v)| (*k, v.clone())).collect();
...
struct LayerVersion {
    layer: Arc<Layer>,
    file_path: PathBuf, // Implementation details
}
...
impl LayerManager {
    ...
    fn rebuild_service_index(
        &self,
        layers_map: &HashMap<String, LayerVersion>,
        ...
        for (service, mut layer_list) in service_to_layers {
            ...
            service_index.insert(service, layer_list.into_iter().map(|(id, _)| id).collect());
        }
    ...
    /// Implementation details
    pub async fn load_layers_from_vec(
        &self,
        layers: Vec<Layer>,
        catalog: &ExperimentCatalog,
    ) -> Result<()> {
        ...
        for layer in layers {
            tracing::info!(
                "Loading layer: {} (version: {}, priority: {})",
                layer.layer_id,
                layer.version,
                layer.priority
            );
            ...
            new_layers.insert(
                ...
                LayerVersion {
                    layer: Arc::new(layer),
                    file_path: PathBuf::new(), // Implementation details
                },
            );
        }
    ...
    /// Implementation details
    pub async fn update_layer(&self, layer: Layer, catalog: &ExperimentCatalog) -> Result<()> {
...
mod tests {
    ...
    fn test_ranges_hit_and_hole() {
        ...
        assert_eq!(layer.get_vid(5000), None); // Implementation details
        assert_eq!(layer.get_vid(7499), None); // Implementation details
    ...
    async fn test_layer_manager_load() {
        ...
        manager
            .load_layers_from_vec(vec![layer], &catalog)
            .await
            .unwrap();
